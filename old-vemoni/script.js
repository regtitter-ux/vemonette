...
const RU = {
  ...
  menu_home: 'Главная', menu_partner: 'Партнёрам', menu_order: 'Покупателям', menu_myorders: 'DMALL', menu_invest: 'Инвесторам', menu_dev: 'Разработчикам', menu_breeders: 'Для ботоводов', menu_admin: 'Администраторам', menu_logout: 'Выйти',
  ...
  what_p1: 'Vemoni - сеть из ботов-верификаторов для крупных Discord-серверов: участники получают полный доступ к серверам, будь то каналы с инсайдами, vip-контентом, закрытыми чатами, или просто эксклюзивные роли за клик, а вы получаете оплату за успешные верификации',
...
(function () {
  ...
  function drawFloat(fl) {
    ...
    if (fl.kind === 'money') { ctx.font = '800 18px Roboto,system-ui,sans-serif'; ctx.textAlign = 'center'; ctx.fillStyle = GREEN; ctx.fillText('+$', p[0], y); }
    else { const s = 18; ctx.textAlign = 'right'; ctx.fillStyle = TRAFFIC; ctx.fillText('+', p[0] - s / 2 - 2, y); if (usersOk) ctx.drawImage(users, p[0] - s / 2, y - s / 2, s, s); else groupGlyph(p[0], y, 9, TRAFFIC); }
    ...
    for (let i = PARTS.length - 1; i >= 0; i--) { const pt = PARTS[i]; pt.t += pt.sp;
      ...
      if (v[2] > -0.05) {
        ...
        if (v[2] > 0.15) { ctx.save(); ctx.globalAlpha = Math.min(1, (v[2] - 0.15) * 3);
          ...
          else if (usersOk) { ctx.globalCompositeOperation = 'source-over'; const s = 18; ctx.drawImage(users, p[0] - s / 2, p[1] - 15 - s / 2, s, s); } // bot -> buyer: members
