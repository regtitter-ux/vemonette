...
(function () {
  ...
  function lotCard(l) {
    ...
    const menu = (l.mine || window.__VEMONI_DM_STAFF__) ? (
      ...
      '<span class="dm-lot-menu" data-lot-menu-for="' + esc(l.id) + '">' +
        ...
        ((window.__VEMONI_DM_STAFF__ && l.creatorId) ? '<span class="dm-lot-mi" data-lot-goto="' + esc(l.creatorId) + '" data-dm="lot_goto_owner">Go to owner</span>' : '') +
  ...
  function cabOrderRow(o) {
    ...
    return '<div class="dmc-row dmc-row-click" data-order="' + esc(o.id || '') + '" role="button" tabindex="0"><div class="dmc-row-l">' + cabAv(o) +
  ...
  { const ob = $('#dmc-orders'); if (ob) ob.addEventListener('click', (e) => {
      const b = e.target.closest('[data-pg]'); if (b) { const n = +b.dataset.pg; if (n >= 1) { dmcOrdersPage = n; renderCabOrders(); } return; }
      const row = e.target.closest('[data-order]'); if (row && row.dataset.order) openOrderModal(row.dataset.order);
    }); }
  ...
  /* ---------- Order details modal ---------- */
  function ordEmbedPreview(e) {
    if (!e) return '';
    const bar = (typeof e.color === 'number') ? '#' + (e.color & 0xffffff).toString(16).padStart(6, '0') : (typeof e.color === 'string' && e.color ? e.color : '#4b5563');
    const author = e.author && e.author.name ? '<div class="dm-ord-em-author">' + (e.author.icon_url ? '<img src="' + esc(e.author.icon_url) + '" alt="" loading="lazy">' : '') + esc(e.author.name) + '</div>' : '';
    const title = e.title ? '<div class="dm-ord-em-title">' + esc(e.title) + '</div>' : '';
    const desc = e.description ? '<div class="dm-ord-em-desc">' + esc(e.description).replace(/\n/g, '<br>') + '</div>' : '';
    const fields = Array.isArray(e.fields) && e.fields.length
      ? '<div class="dm-ord-em-fields">' + e.fields.map((f) => '<div class="dm-ord-em-field' + (f.inline ? ' inline' : '') + '"><div class="dm-ord-em-fname">' + esc(f.name || '') + '</div><div class="dm-ord-em-fval">' + esc(f.value || '').replace(/\n/g, '<br>') + '</div></div>').join('') + '</div>'
      : '';
    const img = e.image ? '<img class="dm-ord-em-img" src="' + esc(e.image) + '" alt="" loading="lazy">' : '';
    const thumb = e.thumbnail ? '<img class="dm-ord-em-thumb" src="' + esc(e.thumbnail) + '" alt="" loading="lazy">' : '';
    const footer = e.footer && e.footer.text ? '<div class="dm-ord-em-footer">' + (e.footer.icon_url ? '<img src="' + esc(e.footer.icon_url) + '" alt="" loading="lazy">' : '') + esc(e.footer.text) + '</div>' : '';
    return '<div class="dm-ord-embed" style="border-left-color:' + esc(bar) + '">' + author + title + '<div class="dm-ord-em-row">' + '<div class="dm-ord-em-main">' + desc + fields + '</div>' + thumb + '</div>' + img + footer + '</div>';
  }
  function ordMessagePreview(m) {
    if (!m || (!String(m.content || '').trim() && !(m.embeds || []).length && !(m.components || []).length)) return '<div class="dm-ord-nomsg" data-dm="ord_no_message">—</div>';
    const content = String(m.content || '').trim() ? '<div class="dm-ord-msg-text">' + esc(m.content).replace(/\n/g, '<br>') + '</div>' : '';
    const embeds = (m.embeds || []).map(ordEmbedPreview).join('');
    const btns = (m.components || []).filter((c) => c && c.label).map((c) => '<a class="dm-ord-btn" href="' + esc(c.url || '#') + '" target="_blank" rel="noopener">' + esc(c.label) + '</a>').join('');
    return content + embeds + (btns ? '<div class="dm-ord-btns">' + btns + '</div>' : '');
  }
  function ordAv(name, avatar) {
    const letter = (String(name || '?').trim()[0] || '?').toUpperCase();
    return avatar ? '<span class="dmc-av"><img src="' + esc(avatar) + '" alt="" loading="lazy"></span>' : '<span class="dmc-av dmc-av-txt">' + esc(letter) + '</span>';
  }
  function closeOrderModal() { const m = $('#dm-ord-modal'); if (m) m.remove(); }
  async function openOrderModal(id) {
    closeOrderModal();
    const wrap = document.createElement('div'); wrap.className = 'dm-ord-modal'; wrap.id = 'dm-ord-modal';
    wrap.innerHTML = '<div class="dm-ord-box"><div class="dm-ord-head"><b data-dm="ord_title">Order details</b><button type="button" class="dm-ord-x" data-ord-close aria-label="close">✕</button></div><div class="dm-ord-body"><div class="dm-ord-loading">…</div></div></div>';
    document.body.appendChild(wrap);
    wrap.addEventListener('click', (e) => { if (e.target === wrap || e.target.closest('[data-ord-close]')) closeOrderModal(); });
    const r = await dmApi('/order/dmall/order?id=' + encodeURIComponent(id));
    const body = $('.dm-ord-body', wrap); if (!body) return;
    if (!r.ok || !r.body || !r.body.order) { body.innerHTML = '<div class="dm-ord-nomsg">' + esc((r.body && r.body.error) || dmT('cab_req_fail')) + '</div>'; dmApplyLang(); return; }
    const o = r.body.order, s = o.settings || {}, money = (x) => '$' + (Number(x) || 0).toFixed(2);
    const stTxt = o.status === 'settled' ? dmT('cab_st_settled') : dmT('cab_st_active');
    const row = (k, v) => v == null || v === '' ? '' : '<div class="dm-ord-row"><span class="dm-ord-k" data-dm="' + k + '">' + k + '</span><span class="dm-ord-v">' + v + '</span></div>';
    const rawRow = (label, v) => v == null || v === '' ? '' : '<div class="dm-ord-row"><span class="dm-ord-k">' + esc(label) + '</span><span class="dm-ord-v">' + v + '</span></div>';
    const tg = s.targeting || {}, op = s.options || {};
    const excl = Array.isArray(tg.exclude_ids) ? tg.exclude_ids.length : 0;
    const sec = (dm, html) => '<div class="dm-ord-sec"><div class="dm-ord-sec-h" data-dm="' + dm + '">' + dm + '</div>' + html + '</div>';
    body.innerHTML =
      '<div class="dm-ord-top">' + ordAv(o.server && o.server.name, o.server && o.server.icon) +
        '<div><div class="dm-ord-srv">' + esc((o.server && o.server.name) || (o.server && o.server.id) || '') + '</div>' +
        '<div class="dm-ord-badge">' + esc(stTxt) + (o.finalStatus ? ' · ' + esc(o.finalStatus) : '') + '</div></div></div>' +
      sec('ord_orderer', '<div class="dm-ord-user">' + ordAv(o.orderer && o.orderer.name, o.orderer && o.orderer.avatar) + '<div><div class="dm-ord-uname">' + esc((o.orderer && o.orderer.name) || (o.orderer && o.orderer.id) || '—') + '</div><div class="dm-ord-uid">' + esc((o.orderer && o.orderer.id) || '') + '</div></div></div>' +
        (o.creator ? row('ord_creator', esc(o.creator.name || o.creator.id)) : '')) +
      sec('ord_summary',
        row('ord_created', esc(cabDate(o.createdAt))) + (o.settledAt ? row('ord_settled', esc(cabDate(o.settledAt))) : '') +
        row('ord_delivered', (Number(o.delivered) || 0).toLocaleString() + ' / ' + (Number(o.requested) || 0).toLocaleString()) +
        row('ord_charged', money(o.charge)) + ((Number(o.refunded) || 0) > 0 ? row('ord_refund', money(o.refunded)) : '') +
        row('ord_net', money(o.net)) + row('ord_fee', money(o.fee)) +
        (o.creator ? row('ord_lotprice', money(o.creatorPrice)) + row('ord_creatorpaid', money(o.creatorPaid)) : '')) +
      sec('ord_settings',
        row('ord_limit', (Number(s.messageLimit) || 0).toLocaleString()) +
        (tg.online_priority ? row('ord_priority', esc(String(tg.online_priority))) : '') +
        (excl ? row('ord_exclude', String(excl)) : '') +
        (op.recency_cooldown_hours ? row('ord_cooldown', String(op.recency_cooldown_hours)) : '') +
        (op.exclude_destination_duplicates ? row('ord_dedup', '✓') : '') +
        (s.destinationLink ? rawRow(dmT('ord_destlink'), '<a href="' + esc(s.destinationLink) + '" target="_blank" rel="noopener">' + esc(s.destinationLink) + '</a>') : '') +
        (s.templateId ? row('ord_template', '<code>' + esc(s.templateId) + '</code>') : '')) +
      sec('ord_message', '<div class="dm-ord-msg">' + ordMessagePreview(o.message) + '</div>');
    dmApplyLang();
  }
  ...
  function dmSelectServer(card) {
    ...
    // Reflect the lot owner's minimum purchase size on the count field (prefill if below it).
    { const lot = (dmLots || []).find((l) => l && l.serverId === dmServerId); const min = (lot && !lot.mine) ? (Number(lot.minOrder) || 0) : 0; const ci = $('#dm-l-count'); if (ci) { ci.min = min > 0 ? min : 1; ci.placeholder = min > 0 ? dmT('l_min_ph').replace('{n}', Number(min).toLocaleString()) : ''; if (min > 0 && (Number(ci.value) || 0) < min) ci.value = min; } }
  ...
  if (dmGrid) dmGrid.addEventListener('click', async (e) => {
    ...
    const goto = e.target.closest('[data-lot-goto]');
    if (goto) { e.preventDefault(); e.stopPropagation(); closeLotMenus(); window.open('/partner/?as=' + encodeURIComponent(goto.dataset.lotGoto) + '&tab=xdmall', '_blank'); return; }
  ...
  function openLotModal(lot) {
    ...
    const si = $('#dm-lot-server'), pi = $('#dm-lot-price'), mi = $('#dm-lot-min');
    const h2 = lotModal.querySelector('h2'), go = $('#dm-lot-create');
    if (lot) {
      // Edit mode: server is fixed (the lot's identity); only the price / minimum change.
      if (si) { si.value = lot.serverId || ''; si.readOnly = true; si.classList.add('dm-ro'); }
      if (pi) pi.value = Number(lot.pricePer1k || 0);
      if (mi) mi.value = Number(lot.minOrder || 0);
  ...
  { const go = $('#dm-lot-create'); if (go) go.addEventListener('click', async () => {
      ...
      const price = Math.max(0, Number(($('#dm-lot-price') || {}).value) || 0);
      const minOrder = Math.max(0, Math.floor(Number(($('#dm-lot-min') || {}).value) || 0));
      const st = $('#dm-lot-status'); const setSt = (cls, m) => { if (st) { st.hidden = false; st.className = 'dm-lot-status ' + cls; st.textContent = m; } };
      // Edit mode → update price + minimum (no server change, no bot re-check).
      if (dmEditLotId) {
        go.disabled = true; setSt('pending', dmT('lot_saving'));
        const r = await dmApi('/order/dmall/lot', { method: 'POST', body: { id: dmEditLotId, pricePer1k: price, minOrder } });
        go.disabled = false;
      ...
      const r = await dmApi('/order/dmall/lot', { method: 'POST', body: { serverId: sid, pricePer1k: price, minOrder } });
      go.disabled = false;
      ...
      else if (r.body && r.body.error === 'server-already-listed') setSt('err', dmT('lot_dup'));
      else setSt('err', (r.body && r.body.error) || dmT('lot_fail'));
    }); }
  ...
  async function launchBroadcast() {
    ...
    // Lot owner's minimum purchase size (buying from someone else's lot).
    { const lot = (dmLots || []).find((l) => l && l.serverId === dmServerId); if (lot && !lot.mine && Number(lot.minOrder) > 0 && count < Number(lot.minOrder)) { dmSetStatus('err', dmT('l_below_min').replace('{n}', Number(lot.minOrder).toLocaleString())); return; } }
    ...
    try {
      ...
      const msgPayload = dmBuildPayload(state, avatarRef);   // kept to store with the order (details view)
      const tpl = await dmApi('/order/dmall/op/templates', { method: 'POST', body: msgPayload });
      ...
      const runBody = {
        ...
        options: {},
        _msg: msgPayload   // stored server-side with the order (stripped before the operator call)
      };
      ...
      if (run.body && run.body.error === 'below-min') { dmSetStatus('err', dmT('l_below_min').replace('{n}', Number(run.body.min || 0).toLocaleString())); return; }
  ...
  const DM_TXT = {
    en: {
      ...
      sp_members:"members", sp_bcasts:"dmalls", sp_delivered:"messages delivered", reviews_word:"Reviews", reviews_none:"No reviews yet",
      ...
      rev_leave:"Leave a review", rev_your_review:"Your review", rev_placeholder:"Share your experience…", rev_post:"Post", rev_save:"Save changes", rev_locked:"Only buyers who broadcast to this server can leave a review.",
    ...
    ru: {
      ...
      sp_members:"участников", sp_bcasts:"dmalls", sp_delivered:"сообщений доставлено", reviews_word:"Отзывы", reviews_none:"Пока нет отзывов",
      ...
      rev_leave:"Оставить отзыв", rev_your_review:"Ваш отзыв", rev_placeholder:"Поделитесь впечатлением…", rev_post:"Отправить", rev_save:"Сохранить изменения", rev_locked:"Оставить отзыв могут только те, кто заказывал рассылку на этот сервер.",
