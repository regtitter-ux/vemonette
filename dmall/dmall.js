...
(function () {
  ...
  // Actionable toast: a message + one action button (used when a broadcast fails → offer another server).
  let dmActToastT;
  function dmActionToast(msg, btnLabel, onAction) {
    let el = $('#dm-act-toast');
    if (!el) { el = document.createElement('div'); el.id = 'dm-act-toast'; el.className = 'dm-act-toast'; document.body.appendChild(el); }
    el.innerHTML = '<span class="dm-at-msg"></span><button class="dm-at-btn"></button><button class="dm-at-x" aria-label="close"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18"/></svg></button>';
    el.querySelector('.dm-at-msg').textContent = msg;
    const b = el.querySelector('.dm-at-btn'); b.textContent = btnLabel;
    b.onclick = () => { el.hidden = true; if (onAction) onAction(); };
    el.querySelector('.dm-at-x').onclick = () => { el.hidden = true; };
    el.hidden = false;
    clearTimeout(dmActToastT); dmActToastT = setTimeout(() => { el.hidden = true; }, 14000);
  }
  // A broadcast didn't go through (funds refunded) → suggest picking another server.
  function dmOfferAnotherServer() { dmActionToast(dmT('fail_notice'), dmT('other_offers'), dmOpenPicker); }
    ...
    async function tick() {
      ...
      if (r.ok && r.body && r.body.run) {
        ...
        if (['completed', 'failed', 'stopped'].includes(run.status)) {
          ...
          if (run.status === 'failed' || sent === 0) dmOfferAnotherServer();
          return;
        }
      }
  ...
  async function launchBroadcast() {
    ...
    try {
      ...
      if (!run.ok || !run.body || !run.body.run) { dmSetStatus('err', dmT('l_run_err') + ' ' + ((run.body && (run.body.message || run.body.error)) || run.status)); dmOfferAnotherServer(); return; }
