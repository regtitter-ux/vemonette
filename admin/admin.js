...
const TR = {
  ...
  // Map — traffic-route bindings
  'Моя ссылка (куда ведём)':'My link (destination)','Источники трафика':'Traffic sources',
  '＋ Новая связка':'＋ New binding','Новая связка':'New binding','＋ добавить сервер-источник…':'＋ add a source server…',
  '⏸ Остановить':'⏸ Stop','▶ Продолжить':'▶ Resume','Остановить':'Stop','Продолжить':'Resume',
  'Пока пусто — добавь сервер справа':'Empty — add a server on the right',
  'Изменений ссылки ещё не было.':'No link changes yet.','Списано:':'Charged:','Доставлено:':'Delivered:',
  'Лимит заходов':'Join limit','Цена/100 зах., $':'Price /100 joins, $','Сохранить':'Save','Удалить':'Delete',
  '(лимит достигнут)':'(limit reached)','(недостаточно средств)':'(insufficient funds)',
  'Оценка по кликам × конверсии':'Estimate from clicks × conversion','Убрать':'Remove',
  'Связка сохранена':'Binding saved','Связка запущена':'Binding started','Связка остановлена':'Binding stopped',
  'Связка удалена':'Binding deleted','Удалить связку?':'Delete this binding?',
  'Связка создана — теперь добавь источники и запусти':'Binding created — now add sources and start it',
  'Некорректная ссылка':'Invalid link','Не удалось прочитать ссылку':'Could not read the link',
  'Пока нет связок. Нажми «Новая связка» и вставь ссылку на сервер, куда вести трафик.':'No bindings yet. Click “New binding” and paste a link to the server to drive traffic to.',
  'Вставь ссылку на сервер, куда вести трафик (https://discord.gg/…):':'Paste a link to the server to drive traffic to (https://discord.gg/…):',
  'Баланс:':'Balance:','ср. конверсия сети':'network avg conversion','Заходы:':'Joins:',
  ...
  'Долг сервиса (положит. балансы)':'Service debt (positive balances)','Баланс Crypto Pay':'Crypto Pay balance','Баланс NOWPayments':'NOWPayments balance',
...
function renderSysFinance(f) {
    ...
    const cards = [
        ...
        { k: 'Баланс NOWPayments', v: f.nowpaymentsBalance == null ? '—' : m(f.nowpaymentsBalance) },
...
function openServerAdModal(gid) {
    ...
    if (isOwner) {
        ...
        <div class="setting wide" style="margin-top:16px;border-top:1px solid rgba(255,255,255,.08);padding-top:14px;">
          ...
          <p class="muted" style="margin:6px 0 0;">Разрешает на этом сервере рекламу без проверки на заход: оплата за клик считается по конверсии, <b>измеренной калибровкой</b> (кнопка «Калибровка»). Пока сервер не откалиброван — берётся средняя конверсия по калиброванным серверам. Пассивная реклама конверсию больше не меняет. Выкл — обычная оплата за заход.</p>
        </div>`;
    }
    let extraAdBlock = '';
    if (isOwner) {
...
async function renderCalibration() {
    ...
    if (rateBox) rateBox.innerHTML = `
      <div class="calib-rate">
        ...
        <p class="muted sm">${s.calibrationConfigured ? '✅ Настроено. ' : '⚠️ Не настроено — без ссылки кнопка «Калибровка» не запустится. '}Кнопка «Калибровка» запускает проверочную рекламу <b>без обязательного захода</b> — юзер получает верифку сразу, как в рекламе без проверки (высший приоритет, партнёр не может скрыть, цель — 100 отслеженных заходов). Наш бот на сервере-спонсоре сам следит, кто реально зашёл и вышел, и считает <b>точную конверсию</b> — так учитываются все, а не только те, кто вернулся на второй клик. Партнёру начисляется за <b>каждый реально отслеженный заход</b> и списывается за выход (сервис несёт стоимость); верифку за выход не снимаем. Важно: у бота на сервере-спонсоре должен быть intent участников.</p>
      </div>
...
// ---------- Map: traffic-route bindings (owner) ----------
// A binding = "my destination link → source servers". While running, the link shows as
// a NO-CHECK filler on the sources (a real order overrides it), verifies without a join,
// and debits the owner per statistical join (±15% counter error). This tab curates the
// bindings, shows live per-binding join stats, a link-change history, and an animated
// flow of traffic along each binding's threads. Loaded lazily on tab open (never in the
// 3s live loop — that would clobber the editor mid-edit).
let mapData = { bindings: [], sources: [], balance: 0, networkConv: 0, srcLoaded: false };
const _mapUsd = (x) => '$' + (Number(x) || 0).toFixed(2);
...
async function loadMap() {
    const listBox = $('#map-list');
    if (!listBox) return;
    if (!mapData.bindings.length) listBox.innerHTML = '<div class="muted">Загрузка…</div>';
    // Candidate sources are heavier to compute — fetch once, then reuse.
    const reqs = [get('/map')];
    if (!mapData.srcLoaded) reqs.push(get('/map/sources'));
    let mapRes, srcRes;
    try { [mapRes, srcRes] = await Promise.all(reqs); }
    catch (e) { listBox.innerHTML = '<div class="muted">Не удалось загрузить — вероятно, нужен передеплой бэкенда.</div>'; return; }
    if (!mapRes.ok) { listBox.innerHTML = `<div class="muted">Ошибка загрузки (${mapRes.body?.error || '—'}).</div>`; return; }
...
function renderMap() {
    ...
    if (bal) bal.innerHTML = `Баланс: <b>${_mapUsd(mapData.balance)}</b><span class="muted"> · ср. конверсия сети ${mapData.networkConv ? (Math.round(mapData.networkConv * 1000) / 10) + '%' : '—'}</span>`;
    if (!mapData.bindings.length) {
        listBox.innerHTML = '<div class="map-empty muted">Пока нет связок. Нажми «Новая связка» и вставь ссылку на сервер, куда вести трафик.</div>';
        return;
    }
...
// Build the animated SVG: each source flows from the left, threads merge at a mid point,
// then one thread runs to the destination on the right. Dots animate only while running.
...
function mapSrcChip(gid) {
    ...
    const est = s ? `~${_mapNum(s.est?.day)}/д` : '';
    return `<span class="map-chip" data-gid="${escapeHtml(gid)}"><span class="map-chip-name" title="${escapeHtml(name)}">${escapeHtml(name)}</span>${est ? `<span class="map-chip-est">${est}</span>` : ''}<button class="map-chip-x" data-act="rm-src" data-gid="${escapeHtml(gid)}" title="Убрать">✕</button></span>`;
}

function mapCardHtml(b) {
    const dest = b.destName || b.destGuildId || '—';
    const destIcon = b.destIcon ? `<img class="map-dest-ic" src="${escapeHtml(b.destIcon)}" alt="" />` : '<span class="map-dest-ic ph"></span>';
    const selected = new Set(b.sources || []);
    const opts = mapData.sources.filter((s) => !selected.has(s.gid) && s.gid !== b.destGuildId)
        .map((s) => `<option value="${escapeHtml(s.gid)}">${escapeHtml(s.name || s.gid)} — ~${_mapNum(s.est?.week)}/нед · ${s.memberCount || 0} уч.</option>`).join('');
    const stopReason = b.stoppedReason === 'limit' ? ' (лимит достигнут)' : b.stoppedReason === 'funds' ? ' (недостаточно средств)' : '';
    const prog = b.limit > 0 ? `${b.delivered} / ${b.limit}` : `${b.delivered}`;
    return `<div class="map-card ${b.running ? 'running' : 'stopped'}" data-id="${escapeHtml(b.id)}">
      <div class="map-row">
        <div class="map-col map-col-left">
          <label class="map-label">Моя ссылка (куда ведём)</label>
          <input class="map-link-input" data-act="link" value="${escapeHtml(b.invite)}" placeholder="https://discord.gg/..." spellcheck="false" />
          <div class="map-dest-info">${destIcon}<span><b>${escapeHtml(dest)}</b>${b.destGuildId ? `<span class="muted"> · id ${escapeHtml(b.destGuildId)}</span>` : ''}</span></div>
          <button class="btn-mini" data-act="history">История ссылок (${(b.history || []).length})</button>
        </div>
        <div class="map-col map-col-flow">${mapFlowSvg(b)}</div>
        <div class="map-col map-col-right">
          <label class="map-label">Источники трафика (${(b.sources || []).length})</label>
          <div class="map-sources">${(b.sources || []).map(mapSrcChip).join('') || '<span class="muted">Пока пусто — добавь сервер справа</span>'}</div>
          <select class="map-src-add" data-act="add-src"><option value="">＋ добавить сервер-источник…</option>${opts}</select>
        </div>
      </div>
      <div class="map-history" hidden>${(b.history || []).slice().reverse().map((h) => `<div class="map-hist-row"><span class="map-hist-link">${escapeHtml(h.invite)}</span><span class="muted">${escapeHtml(h.destName || h.destGuildId || '')}</span></div>`).join('') || '<span class="muted">Изменений ссылки ещё не было.</span>'}</div>
      <div class="map-foot">
        <button class="map-toggle ${b.running ? 'on' : 'off'}" data-act="toggle">${b.running ? '⏸ Остановить' : '▶ Продолжить'}</button>
        <span class="map-stat" title="Оценка по кликам × конверсии">Заходы: <b>${_mapNum(b.joins?.hour)}</b>/ч · <b>${_mapNum(b.joins?.day)}</b>/д · <b>${_mapNum(b.joins?.week)}</b>/нед <i class="map-err">±15%</i></span>
        <span class="map-progress">Доставлено: <b>${prog}</b>${stopReason}</span>
        <label class="map-inl">Лимит заходов <input type="number" min="0" step="50" data-act="limit" value="${b.limit || 0}" /></label>
        <label class="map-inl">Цена/100 зах., $ <input type="number" min="0" step="0.5" data-act="price" value="${b.pricePer100 || 0}" /></label>
        <span class="map-spent">Списано: <b>${_mapUsd(b.spentUsd)}</b></span>
        <button class="btn primary sm" data-act="save">Сохранить</button>
        <button class="btn-mini off" data-act="del">Удалить</button>
      </div>
    </div>`;
}

// Read the editable fields of every card back into the in-memory model, so a re-render
// (triggered by adding/removing a source) never loses an unsaved link/limit/price edit.
...
function wireMapCard(id) {
    ...
    card.querySelectorAll('[data-act="rm-src"]').forEach((x) => x.onclick = () => {
        mapCollectEdits();
        b.sources = (b.sources || []).filter((g) => g !== x.dataset.gid);
        renderMap();
    });

    q('[data-act="toggle"]').onclick = async () => {
        mapCollectEdits();
        // Turning ON also persists the current edits, so "Продолжить" never runs a stale link/sources set.
        ...
        if (ok) { mapReplace(body.binding); toast(b.running ? 'Связка остановлена' : 'Связка запущена'); renderMap(); }
        else toast(body?.error === 'bad-invite' ? 'Некорректная ссылка' : (body?.error || 'Не удалось'), 'err');
    };
    ...
    q('[data-act="save"]').onclick = async () => {
        ...
        btn.disabled = false;
        if (ok) { mapReplace(body.binding); toast('Связка сохранена'); renderMap(); }
        else toast(body?.error === 'bad-invite' ? 'Некорректная ссылка' : (body?.error || 'Не удалось'), 'err');
    };
    ...
    q('[data-act="del"]').onclick = async () => {
        ...
        if (ok) { mapData.bindings = mapData.bindings.filter((x) => x.id !== id); toast('Связка удалена'); renderMap(); }
        else toast(body?.error || 'Не удалось', 'err');
    };
}
...
const _mapAddBtn = document.getElementById('map-add');
if (_mapAddBtn) _mapAddBtn.onclick = async () => {
    const invite = prompt('Вставь ссылку на сервер, куда вести трафик (https://discord.gg/…):');
    if (invite == null) return;
    const v = invite.trim(); if (!v) return;
    _mapAddBtn.disabled = true;
    const { ok, body } = await post('/map', { invite: v });
    _mapAddBtn.disabled = false;
    if (ok) { mapReplace(body.binding); toast('Связка создана — теперь добавь источники и запусти'); mapData.srcLoaded = false; loadMap(); }
    else toast(body?.error === 'bad-invite' ? 'Не удалось прочитать ссылку' : (body?.error || 'Не удалось'), 'err');
};
const _mapTab = document.querySelector('[data-tab="map"]');
if (_mapTab) _mapTab.addEventListener('click', loadMap);
// Live-refresh join counters while the tab is open and something is running (read-only
// fields, so it won't disturb editing — link/limit/price are re-collected first).
setInterval(() => {
    if (document.querySelector('[data-pane="map"]')?.hidden) return;
